	...
	void Start () {
		...
		if (iterations >= 1) {
			for (int i = 0; i < splits; i++) {
				...
				Tree instance = Instantiate (twigReference, transform.position + transform.forward * twiglength, twigRotation) as Tree;
...
//		Debug.Log (goalRotation);
		...
		if (!doneParenting) {
			...
			//			Quaternion randomRotation = Random.rotation;
			//			transform.rotation = Quaternion.RotateTowards (originalRotation, goalRotation, Random.Range (0.0f, windAngle * Time.deltaTime));
			transform.rotation = Quaternion.RotateTowards (transform.rotation, maxRotation, Random.Range (0.0f, windAngle * Time.deltaTime / 10));
